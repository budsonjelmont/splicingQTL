# Make QTL analysis files from PEC isoQTL data 
import pandas as pd

# New WASP-treated data
resdir = '/sc/arion/projects/EPIASD/splicingQTL/analysis/annotationBEDs/PECisoQTLs/hg19/'
resfile = 'DER-10a_hg19_isoQTL.significant.txt'
...
# Identify rows where SNP ID is not blank 
...
# Also export sQTL SNP bed file of just best hits for each pheno (THIS is the file to use for GREGOR)
res.drop_duplicates('pid').drop_duplicates('sid').to_csv(resdir+'uniqueLeadSNPuniquephenos-chrpos.txt',sep='\t',header=False,index=False,columns=['sid'])
# Drop duplicate phenotypes and write list of all SNPs
res.drop_duplicates('sid').to_csv(resdir+'uniqueSNPs-chrpos.txt',sep='\t',header=False,index=False,columns=['sid'])
