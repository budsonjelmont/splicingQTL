...
def collapse_annotation(annot, transcript_gtf, collapsed_gtf, blacklist=set(), collapse_only=False):
    ...
    for g in annot.genes:
        ...
        for t in g.transcripts:
            if (t.id not in blacklist) and (('tags' not in t.attributes) or len(set(t.attributes['tags']).intersection(exclude))==0):
                for e in t.exons:
    ...
    with open(collapsed_gtf, 'w') as output_gtf, opener as input_gtf:
        ...
        for line in input_gtf:
            if line[:2]=='##' or line[:2]=='#!' or line[:1]=='#': # added last condition to recognize single '#'-commented lines
                output_gtf.write(line)
                comment = line[:2]
            else:
                comment = '' # added to avoid breaking on comment-less GTFs
                break
