abovebelow0_bar = function(dat,fill,filltitle,ycat,yname,xcounts,xname,barlabel,outpath){
  p=ggplot(data = dat, aes_string(fill = fill, x = xcounts, y = ycat), width = 0.6) +
    ...
    xlab('') +
    scale_y_continuous(name = yname, limits=c(-2,2), breaks=seq(-2,2,by=0.5), expand = c(0,.1)) +
